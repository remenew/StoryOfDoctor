 ...
 * @returns {Array} 草药列表[{name}]
 */
function parseComposition(composition) {
  const herbs = [];
  
  if (!composition) return herbs;
  
  // 解析组成字段，格式: "麻黄去节，三两（9g）、桂枝去皮，二两（6g）..."
  // 按顿号分隔不同草药
  const herbEntries = composition.split(/[、，,]/);
  ...
  for (const entry of herbEntries) {
    ...
    if (match) {
      let herbName = match[1].trim();
      // 去除常见的炮制后缀
      herbName = herbName.replace(/(去节|去皮|去皮尖|炙|切|擘|如鸡子大)$/, '').trim();
      
      if (herbName && herbName.length >= 1) {
        herbs.push({ name: herbName });
      }
    }
  }
 ...
 * @returns {Object} 角色映射 {草药名: 角色}
 */
function parseRoles(analysis) {
  const roles = {};
...
async function main() {
  ...
  for (let i = 1; i < formulaLines.length; i++) {
    ...
    if (fields.length >= 9) {
      ...
      const analysis = fields[7]; // 方解（君臣佐使）- 第8列
      
      if (formulaName) {
        const herbs = parseComposition(composition);
        const roles = parseRoles(analysis);
        
        // 为每个草药分配角色和攻击力
        const herbsWithRoles = herbs.map(h => {
          const role = roles[h.name] || '佐'; // 默认佐药
          return {
            name: h.name,
            role: role,
  ...
  // 打印一些方剂示例
  const formulaNames = Object.keys(formulaData).slice(0, 5);
  console.log('\n方剂示例:');
  for (const name of formulaNames) {
    const f = formulaData[name];
    console.log(`  ${name}: ${f.herbs.map(h => `${h.name}(${h.role}:${h.attack})`).join(', ')}`);
  }
  ...
  for (let i = 1; i < diseaseLines.length; i++) {
    ...
    if (fields.length >= 8) {
      ...
      const treatment = fields[6]; // 治法
      const formula = fields[7]; // 代表方剂
